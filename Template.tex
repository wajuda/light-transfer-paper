...
Let $Z \in \mathbb{R}^{C \times N}$ ($N = HW$) denote a reshaped latent feature and $Z_c = Z - \bm{\mu}(Z)$ its centered version. Its covariance admits a Cholesky factorization
\begin{equation}
    \bm{\Sigma}_Z = Z_c Z_c^{\!\top} = \bm{L}\, \bm{L}^{\!\top},
\end{equation}
where $\bm{L} \in \mathbb{R}^{C \times C}$ is lower-triangular with non-negative diagonal. The transfer operation is then
\begin{equation}
\begin{split}
    Z_{\text{transfer}} &= \mathcal{T}(Z_{\text{night}}, Z_{\text{day}}) \\
    &= \bm{L}_{\text{day}}\, \bm{L}_{\text{night}}^{-1}\,(Z_{\text{night}} - \bm{\mu}(Z_{\text{night}})) + \bm{\mu}(Z_{\text{day}}),
\end{split}
\end{equation}
where $\bm{L}_{\text{night}}^{-1}$ and $\bm{L}_{\text{day}}$ play the roles of whitening and coloring, respectively. In practice we never explicitly invert $\bm{L}_{\text{night}}$; instead we solve a triangular system, which is numerically stable as long as the Cholesky factor is well-conditioned.
...
Let $\bm{L}_c$ and $\bm{L}_s$ be the Cholesky factors of the regularized content covariance and the style covariance, respectively. The transport matrix that maps centered content features to style-coloured features is
\begin{equation}
    \bm{T} = \bm{L}_c^{-1}\, \bm{L}_s.
\end{equation}
