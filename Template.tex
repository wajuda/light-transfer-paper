...
Let $Z \in \mathbb{R}^{C \times N}$ ($N = HW$) denote a reshaped latent feature and $Z_c = Z - \bm{\mu}(Z)$ its centered version. Its covariance matrix admits an SVD-like eigendecomposition
\begin{equation}
    \bm{\Sigma}_Z = Z_c Z_c^{\!\top} = U\, \mathrm{diag}(\sigma_1, \ldots, \sigma_C)\, U^{\!\top},
\end{equation}
where $U \in \mathbb{R}^{C \times C}$ is orthogonal and $\sigma_i \geq 0$ are the eigenvalues (squared singular values of $Z_c$). The transfer operation is then
\begin{equation}
\begin{split}
    Z_{\text{transfer}} &= \mathcal{T}(Z_{\text{night}}, Z_{\text{day}}) \\
    &= U_{\text{day}}\, \mathrm{diag}(\sigma_{\text{day},i}/\sigma_{\text{night},i})\, U_{\text{night}}^{\!\top}\,(Z_{\text{night}} - \bm{\mu}(Z_{\text{night}})) + \bm{\mu}(Z_{\text{day}}),
\end{split}
\end{equation}
where $U_{\text{night}}^{\!\top}$ and $U_{\text{day}}$ play the roles of whitening and coloring, respectively.

When the input is severely degraded, the covariance of $Z_{\text{night}}$ is ill-conditioned: some $\sigma_{\text{night},i}$ are close to zero, so that $1/\sigma_{\text{night},i}$ amplifies noise and produces colour artefacts and instability. To address this, we propose a \emph{Regularized WCT} (RegWCT) that stabilizes the spectrum of $Z_{\text{night}}$ with two closed-form operations.

\noindent\textbf{Covariance shrinkage.}
We mix the empirical covariance with a scaled identity,
\begin{equation}
    \widetilde{\bm{\Sigma}}_Z = (1 - \alpha)\, \bm{\Sigma}_Z + \alpha\, \tfrac{\mathrm{tr}(\bm{\Sigma}_Z)}{C}\, \bm{I}, \qquad \alpha \in [0, 1),
\end{equation}
which equivalently replaces every eigenvalue by
\begin{equation}
    \widetilde{\sigma}_i = (1 - \alpha)\, \sigma_i + \alpha\, \tfrac{\mathrm{tr}(\bm{\Sigma}_Z)}{C}.
\end{equation}
The same $U$ is reused, since $U$ commutes with $\bm{I}$.

\noindent\textbf{Singular-value clipping.}
We further bound the dynamic range of the spectrum by clipping each eigenvalue into $[s_{\min}, s_{\max}]$,
\begin{equation}
    \widehat{\sigma}_i = \mathrm{clip}\!\left(\widetilde{\sigma}_i,\; s_{\min},\; s_{\max}\right).
\end{equation}
The regularized whitening matrix is then $U_{\text{night}}\, \mathrm{diag}(1/\widehat{\sigma}_{\text{night},i})\, U_{\text{night}}^{\!\top}$.
