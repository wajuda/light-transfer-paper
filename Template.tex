...
The architecture diagram of the proposed RLTNet is illustrated in Fig.~\ref{fig:structure}. mainly three parts 
\subsubsection{Reversible Network}

why we need reversible. simple encode decoder used in style transfer 容易导致失真扭曲，一般用于艺术风格迁移，realisetic style transfer need other 专心设计的损失。However, the reversible network promise the perfect latent to rgb process.



how to be reversible
y2 = x2 + E (x1)
y1 = x1 + E(y2)

x1 = y1 - E(y2)
x2 = y2- E (x1)
where E is Extractor module. which contains three conv block.

the 1 is channel low  projector , 3 back. Then second use the dilated to add the receptice field.

before the reversible network , we use a frozen illumination mudule to I_night, initial enhance, make it more similar 分布 as I_day
\subsubsection{Regularzied WCT}

After the reversible , we get the latent represetation $Z_{day}$ and $Z_{night}$.  we adopt the WCT (\cite{capvst}), to match the inter-correlation. 
...
\end{equation}
 ...
 covariance shrinkage + singular-value clipping 数值稳定，限制幅度，  清晰公式展示
 
 \subsubsection{Refiner Network}

I_enhanced = \alpha I_stylized + (1 - \alpha) I_illuminated
dual path refine network . one is channel path use the SEBlock, another is spatial path use stacked conv layers. we concat , conv then sigmoid to get \alpha 1*H*W
